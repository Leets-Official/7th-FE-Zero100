...
export const List = ({tasks, checkHandler, editTask, deleteTask, editingId, cancelEditTask}) => {
        ...
        <ul>
          {tasks.map((task) =>
            <li key={task.id} className='w-94 px-3.5 py-2.5 mb-4 outline outline-gray-300 rounded'>
              {editingId === task.id ? 
                <div><Input defaultValue="" placeholder={`New name for ${task.taskName}`} className='px-1 outline outline-gray-300 rounded' value={editText} onChange={e => setEditText(e.target.value)}/></div>
                :
              ...
              <div className='mt-2 mb-1'>
                ...
                {editingId === task.id ?
                  <Button className='w-42 inline-block h-7.5 bg-red-500 text-white rounded' onClick={()=> cancelEditTask()}> Cancel </Button>
                  :
